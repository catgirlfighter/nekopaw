...
function SetTaskbarOverlayIcon(const AList: TImageList;
  const IconIndex: Integer; const ADescription: String): Boolean; overload;
...
begin
  ...
  begin
    ...
    Result := GlobalTaskBarInterface.SetProgressState(Application.Handle,
      Flag) = S_OK;
  end;
end;
...
begin
  ...
  if CheckAPI then
  begin
    Result := GlobalTaskBarInterface.SetProgressValue(Application.Handle,
      ACurrent, AMax) = S_OK;
  end;
end;
...
begin
  ...
  if CheckAPI then
  begin
    Result := GlobalTaskBarInterface.SetOverlayIcon(Application.Handle, AIcon,
...
if (Win32MajorVersion >= 6) and (Win32MinorVersion > 0) and InitializeTaskbarAPI
then
